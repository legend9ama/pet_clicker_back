...
class UserFarm(Base):
    ...
    current_income = Column(Float)
    current_upgrade_cost = Column(Float)
...
def calculate_values(mapper, connection, target):
    if target.farm_template:
        target.current_upgrade_cost = target.farm_template.base_price * (target.farm_template.price_multiplier ** target.level)
        target.current_income = target.farm_template.base_income * (target.farm_template.income_multiplier ** target.level)